...
class YoloRealSense:
    def __init__(self, model_path, output_csv_path):
        # Load the YOLO model
        self.model = YOLO(model_path)
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.model.to(self.device)
        
        # Ensure the model uses float32 (not half precision)
        self.model.float()  # Force the model to use float32 precision
        ...
        self.fps_data = []  # To store FPS data for saving to CSV
        self.frame_time_data = []  # To store frame time data
    ...
    def process_images(self):
        while True:
            try:
                ...
                if elapsed_time >= self.recording_interval:  # Update every 10 seconds
                    ...
                    self.fps_data.append(fps)  # Save FPS data for CSV
                    self.frame_time_data.append(elapsed_time / self.frame_count * 1000)  # Average frame time in ms
                ...
                # Display Time, FPS Update number, and Frame Time
                cv2.putText(frame, f"Time: {elapsed_seconds} sec | FPS Update: {fps_update_num}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                cv2.putText(frame, f"Frame Time: {elapsed_time / self.frame_count * 1000:.3f} ms", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                ...
                if cv2.waitKey(1) & 0xFF == ord('q') or self.record_count >= 6:  # Stop after 60 seconds for demo
                    break
        ...
        # After processing, save FPS and frame time data to a CSV file
        try:
            with open(self.output_csv_path, 'w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(["Interval", "FPS", " Frame Time"])
                
                # Compute average FPS and frame time for the entire duration
                average_fps = np.mean(self.fps_data)
                average_frame_time = np.mean(self.frame_time_data)
                
                # Save the data for each interval
                for i in range(len(self.fps_data)):
                    writer.writerow([i + 1, self.fps_data[i], self.frame_time_data[i]])
                
                # Save the overall average values
                writer.writerow(["Average", average_fps, average_frame_time])
                
                print(f"FPS and frame time data saved to {self.output_csv_path}")
        except Exception as e:
            print(f"Error saving FPS and frame time data to CSV: {e}")


if __name__ == "__main__":
    model_path = r"D:\123\123main\pt\best (shufflnetv2).pt"  # Replace with your actual YOLO model path
    output_csv_path = r"D:\123\123main\fps\fps_data.csv" # Path to save the FPS data
    yolo_realsense = YoloRealSense(model_path, output_csv_path)
